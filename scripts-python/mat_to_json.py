...
def matlab_datenum_to_datetime(matlab_datenum):
    """
    Converts a MATLAB datenum to a Python datetime.date object.
    """
...
def mat_to_json(stockcode, input_dir, output_dir):
    """
    Processes a single stock's .mat files and generates a detailed JSON file.
    """
    ...
    # Simplified MAT file paths
    dataname_base = f"optout_{stockcode}_h{opth}_hsd{hnumsd}_nstep{nstep}"
    reference_file_path = os.path.join(input_dir, f"{dataname_base}.mat")

    dataname2_path = os.path.join(input_dir, f"optout_{stockcode}_splitadj_h{opth}_hsd{hnumsd}_nstep{nstep}.mat")
    ...
    if ti.size > 0:
        for t in range(nperiod):
            ...
            for i in range(ntaugp):
                # Corrected the logic to not multiply by 365, as per code review
    ...
    json_filename = f"bubble_data_{stockcode}_{yr2}.json"
