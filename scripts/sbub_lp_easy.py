...
# ========= Importing Count Data =========
    ...
    for t in range(nperiod):
        ...
        +       upbd)
            ...
            # FIX 1: Enforce monotonicity (non-decreasing) throughout qcdfp.
            # The put CDF must be non-decreasing everywhere, but local polynomial
            # smoothing can produce local inversions. A left-to-right running
            # maximum (isotonic-style) corrects these without distorting the
            # integral used to recover qcdfp_mu, and prevents qcdfp_mu from
            # overshooting sout[t] (which would produce a negative bubble).
            for i in range(1, nxp):
                if qcdfp[i] < qcdfp[i - 1]:
                    qcdfp[i] = qcdfp[i - 1]
            ...
            if qcdfp_range > minrange and np.max(dqcdfp) < maxcdfjump:
                ...
                # NOTE: Do NOT clamp qcdfp_mu to sout[t] here.
                # qcdfp_mu is the tail-truncated fundamental value μ̂†.
                # The bias correction B̂ (qcdfp_bias) is added in the plotting
                # stage. Clamping here would corrupt that downstream correction.
        ...
        +        upbd)
            ...
            # FIX 2: Enforce monotonicity (non-decreasing) throughout qcdfc.
            # Call prices are non-increasing in strike, so the call-implied CDF
            # (1 + bgc[1]/upbd) must be non-decreasing. Local polynomial fitting
            # can violate this, and after clip+boundary propagation, interior
            # inversions remain. A running-maximum pass corrects them, preventing
            # qcdfc_mu from overshooting sout[t] and producing a negative bubble.
            for i in range(1, nxc):
                if qcdfc[i] < qcdfc[i - 1]:
                    qcdfc[i] = qcdfc[i - 1]
            ...
            if qcdfc_range > minrange and np.max(dqcdfc) < maxcdfjump:
                ...
                # NOTE: Do NOT clamp qcdfc_mu to sout[t] here.
                # qcdfc_mu is the tail-truncated fundamental value μ̂†_c.
                # The bias correction B̂ (qcdfc_bias) is added in the plotting
                # stage. Clamping here would corrupt that downstream correction.
                ...
                total_variance = np.nansum(qcdfc_norm_se_pt**2) + 2 * nw_cov(qcdfc_pt, mc)
            ...
            # Compute raw bubble estimates (S_t - μ̂†).
            # NOTE: These are the un-bias-corrected estimates. The bias term B̂
            # (qcdfp_bias / qcdfc_bias / qcdf_bias) is added in the plotting
            # stage to produce the final Π̂^S = S - μ̂† + B̂ (equation 18 of
            # Jarrow & Kwok (2021)).
            #
            # On negative values: Proposition 1 of Jarrow & Kwok says Π^S ≥ 0
            # under NFLVR, but the authors themselves (Figure 6, p.14) observe
            # negative bias-corrected estimates during 2009 and 2011, attributing
            # them to market frictions/illiquidity that violate the frictionless
            # and competitive market assumptions. Negative estimates are therefore
            # informative signals about stressed market conditions, not pure errors.
            #
            # The CDF monotonicity fix above (Fixes 1 & 2) removes the primary
            # numerical source of spurious negatives. If you additionally want to
            # hard-clamp the raw estimates to zero, set clamp_negative=True below.
            clamp_negative = False   # set True to suppress negatives entirely
            raw_p   = sout[t] - qcdfp_mu
            raw_c   = sout[t] - qcdfc_mu
            raw_cp  = sout[t] - qcdf_mu
            sbub_qcdfp[t, j] = max(0.0, raw_p)  if clamp_negative else raw_p
            sbub_qcdfc[t, j] = max(0.0, raw_c)  if clamp_negative else raw_c
            sbub_qcdf[t, j]  = max(0.0, raw_cp) if clamp_negative else raw_cp
